 ...
 *
 * Usage:
 * 1. Navigate to a YouTube/Vimeo video page
 * 2. Click "Get Data" to collect thumbnail URLs
 * 3. Click "Run CD" to get download links
 */
...
function DataCollector(currentUrl) {
  ...
  if (hostname.includes('youtube.com') || hostname.includes('youtu.be')) {
    ...
    if (videoId) {
      ...
      result.message = 'Found ' + result.thumbnails.length + ' thumbnails for YouTube video';
    } else {
...
function Run(collectedData) {
  ...
  // Return all thumbnails with download info
  var result = {
    success: true,
    platform: collectedData.platform,
    videoId: collectedData.videoId,
    downloadLinks: collectedData.thumbnails,
    bestQuality: collectedData.thumbnails[0].url,
    instructions: 'Right-click on a URL and select "Open in new tab", then right-click the image to save.'
  };
